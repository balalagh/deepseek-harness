...
function extractLlmRequests(events) {
  const requests = [];
  let pending = { systemPrompt: '', runtimeContext: '', skillsList: '', messages: [] };
  for (const e of events) {
    if (e.type === 'system/message') {
      pending.systemPrompt = (e.data?.message?.content || []).map(b => b.text || '').join('');
    } else if (e.type === 'user/message') {
      const src = e.data?.source;
      const text = (e.data?.content || []).map(b => b.text || '').join('');
      if ((src?.kind === 'plugin' && src?.plugin === '@deepseek-ai/dsh-system-prompt' && text.startsWith('Current runtime context')) || (src?.kind === 'plugin' && src?.form === 'snapshot')) {
        pending.runtimeContext = text;
      } else if (src?.kind === 'agent-instructions' || src?.kind === 'skill-catalog') {
        pending.skillsList = text;
      } else if (src?.kind === 'user') {
        pending.messages.push({ role: 'user', text });
      }
      ...
      if (text || toolCalls.length) pending.messages.push({ role: 'assistant', text, toolCalls });
    } else if (e.type === 'tool/call') {
      pending.messages.push({ role: 'tool_call', name: e.data?.name, args: e.data?.arguments });
      ...
      pending.messages.push({ role: 'tool_result', text });
      ...
      const req = {
        seq: e.seq, provider: cfg.provider, model: cfg.model, reason: e.data?.reason,
        effort: cfg.reasoningEffort, maxTokens: cfg.maxTokens, tools,
        systemPrompt: pending.systemPrompt, runtimeContext: pending.runtimeContext,
        skillsList: pending.skillsList, messages: [...pending.messages], personaPrefix: ''
      };
      const sp = req.systemPrompt;
      const m = sp.match(/^You are an AI agent powered by[^\n]*\.\n\n([\s\S]*?)(?=\n\nYou can |\n\nTwo planes|$)/);
      req.personaPrefix = m ? m[1].trim() : '';
      requests.push(req);
    }
  }
  return requests;
